  ...
  <q-dialog v-model="isOpen" persistent>
    <q-card style="width: 200px">
        ...
        <q-card-actions align="center">
          <q-btn label="再来" @click="() => onAction('again')" color="positive" v-close-popup />
          <q-btn label="退出" @click="() => onAction('quit')" color="negative" v-close-popup />
        </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
export default defineComponent({
  setup(props) {
    ...
    const open = ({result, isTimeout, action}:
      {result: number, isTimeout: boolean, action: any}) => {
      let textMap: any = {
        0: '平局',
