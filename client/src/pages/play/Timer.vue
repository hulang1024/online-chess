<template>
  <div class="timer">
    ...
    <span>:</span>
    <span class="time" :class="`text-${secondsColor}`">{{displaySeconds}}</span>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    // 用户设置的计时总时间
    let totalSeconds = ref<number>(props.totalSeconds);
    // 计时当前秒数
    const _seconds = ref<number>(totalSeconds.value);
    // 计算显示
    const displayMinutes = computed(() =>
      totalSeconds.value === null ? '--' : padZero(Math.floor(_seconds.value / 60)));
    const displaySeconds = computed(() =>
      totalSeconds.value === null ? '--' : padZero(_seconds.value % 60));
    const secondsColor = computed(() => {
      const sec = _seconds.value;
      if (!sec) return '';//包括了0和null
      else if (sec < totalSeconds.value / 4) return 'red';
      else if (sec < totalSeconds.value / 2) return 'orange';
      return '';
    });
